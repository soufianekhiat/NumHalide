...
// -----------------------------------------------------------------------------
//
// A real signal has a Hermitian spectrum -- X[n-k] = conj(X[k]) -- so half of a
// full DFT of real data is redundant. numpy, scipy and JAX all exploit that:
// rfft returns n/2+1 bins and irfft consumes exactly those.
//
// dft_1d ABOVE DOES NOT DO THIS. Applied to real input it returns all n bins,
// which is a different (valid, redundant) contract. Both exist here on purpose;
// what matters is not mixing them, because their adjoints differ: the transpose
// of the half-spectrum form carries a factor of 2 on every bin that stands for
// a conjugate pair it no longer stores, and the full form does not.

/// @brief DFT of a real signal, returning the HALF spectrum (numpy rfft)
/// @param real_in Real-valued input Func
/// @param n       Logical signal length
/// @param type    Element type
/// @param name    Base name
/// @return Complex Func valid for k in [0, n/2] -- n/2+1 bins
///
/// The remaining bins are recoverable as conj of their mirror, which is exactly
/// what irdft_1d_normalized below assumes. Realize only [0, n/2+1): the Func is
/// defined beyond that but those values are the redundant half.
inline
Halide::Func rdft_1d(Halide::Func real_in, Halide::Expr n, Halide::Type type,
                     std::string const& name = "rdft1d")
{
    Halide::Var i("i");
    Halide::Func packed(name + "_packed");
    packed(i) = complex(Halide::cast(type, real_in(i)),
                        Halide::Internal::make_zero(type));
    packed.compute_root();

    return dft_1d(packed, n, -1, type, name);
}

/// @brief Inverse of rdft_1d: Hermitian half spectrum -> real signal (numpy irfft)
/// @param half Complex Func holding bins [0, n/2]
/// @param n    Logical signal length of the RESULT
/// @param type Element type
/// @param name Base name
/// @return Real Func of length n, normalized by 1/n
///
/// Defined the way the identity is stated rather than by a summation trick:
/// rebuild the full spectrum through X[k] = conj(X[n-k]) for k > n/2, inverse
/// transform, keep the real part. Slower than folding the sum, and it cannot
...
/// domain, so `half(select(k <= n/2, k, n - k))` makes Halide demand `half`
/// over all of [0, n) -- reading past the n/2+1 bins the caller actually has.
/// Under no_bounds_query that returns adjacent heap rather than faulting.
inline
Halide::Func irdft_1d_normalized(Halide::Func half, Halide::Expr n,
                                 Halide::Type type,
                                 std::string const& name = "irdft1d")
{
    Halide::Var m("m");
    Halide::RDom r(0, n, name + "_r");

    Halide::Expr half_n = n / 2;
    Halide::Expr mirror = Halide::clamp(
        Halide::select(r <= half_n, r, n - r), 0, half_n);
    // Conjugate exactly on the reconstructed half.
    Halide::Expr conj_sign =
        Halide::select(r <= half_n, Halide::Internal::make_one(type),
                       Halide::Internal::make_const(type, -1.0));

    Halide::Tuple xk = complex(Halide::cast(type, half(mirror)[0]),
                               conj_sign * Halide::cast(type, half(mirror)[1]));

    Halide::Expr two_pi = Halide::Internal::make_const(type, 2.0 * M_PI);
    Halide::Expr angle =
        two_pi * Halide::cast(type, m) * Halide::cast(type, r) / Halide::cast(type, n);

    Halide::Tuple prod = complex_mul(xk, expj(angle));

    Halide::Func result(name);
    result(m) = Halide::sum(prod[0]) / Halide::cast(type, n);
    result.compute_root();
    return result;
}

/// @brief 2-D DFT of a real image, HALF spectrum along x (numpy rfft2)
/// @return Complex Func valid for x in [0, w/2], y in [0, h)
inline
Halide::Func rdft_2d(Halide::Func real_in, Halide::Expr w, Halide::Expr h,
                     Halide::Type type, std::string const& name = "rdft2d")
{
    Halide::Var x("x"), y("y");
    Halide::Func packed(name + "_packed");
...
/// @brief Inverse of rdft_2d: Hermitian half spectrum -> real image (numpy irfft2)
///
/// Hermitian symmetry in 2-D pairs (kx, ky) with (w-kx, h-ky) -- BOTH axes wrap,
/// not just the halved one. Mirroring only x is the obvious mistake here and it
/// produces a plausible image with a wrong imaginary cancellation.
inline
Halide::Func irdft_2d_normalized(Halide::Func half, Halide::Expr w, Halide::Expr h,
                                 Halide::Type type,
                                 std::string const& name = "irdft2d")
{
    Halide::Var x("x"), y("y");
    ...
    Halide::Expr two_pi = Halide::Internal::make_const(type, 2.0 * M_PI);
    Halide::Expr angle = two_pi * (Halide::cast(type, x) * Halide::cast(type, r.x) / Halide::cast(type, w)
